...
function handleUpdateRequest() {
    ...
    $query = "UPDATE appoints SET";
    //
    if ($branchID != "") { 
        $query = $query . " branchID='". $branchID ."'";
    } 
    
    $query = $query .  " WHERE lawyerID='" . $lawyerID . "'";


    echo $query;
    // you need the wrap the old name and new name values with single quotations
    // executePlainSQL("UPDATE appoints SET name='" . $new_name . "' WHERE name='" . $old_name . "'");
...
function handleInsertRequest() {
    ...
    executeBoundSQL("insert into appoints values (:bind1, :bind2)", $alltuples);
    OCICommit($db_conn);
}
...
function handleDeleteRequest() {
    ...
    $query = "DELETE FROM appoints WHERE agentID='" . $agentID . "'";
    executePlainSQL($query);
...
if (isset($_POST['select']) || isset($_POST['insert']) || isset($_POST['update'])|| isset($_POST['delete'])) {
    ...
    if(connectToDB()) {
        $result = executePlainSQL("select * from appoints");
        printResult($result);
    }
