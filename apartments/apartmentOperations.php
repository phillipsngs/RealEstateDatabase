...
function handleUpdateRequest() {
    ...
    //
    if ($association != "") { 
        $query = $query . " association='".$association."'";
    } else if ($floornumber != "" ) {
        $query = $query . " floornumber='".$floornumber."'";
    }
    $query = $query .  " WHERE propertyid='" . $propertid . "'";
...
function handleInsertRequest() {
    ...
    $tuple = array (
        ":bind1" => $_POST['propertyid'],
        ":bind2" => $_POST['floornumber'],
        ":bind3" => $_POST['association'],
    );
    ...
    executeBoundSQL("insert into apartments values (:bind1, :bind2, :bind3)", $alltuples);
    OCICommit($db_conn);
}
...
//    echo" about to print!";
    if(connectToDB()) {
        $result = executePlainSQL("select * from apartments");
        printResult($result);
    }
