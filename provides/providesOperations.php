...
function handleUpdateRequest() {
    ...
    $query = "UPDATE provides SET";
    //
    if ($moverid != "") { 
        $query = $query . " moverid='".$moverid."'";
    } 
    ...
    // executePlainSQL("UPDATE provides SET name='" . $new_name . "' WHERE name='" . $old_name . "'");
...
function handleInsertRequest() {
    ...
    executeBoundSQL("insert into provides values (:bind1, :bind2)", $alltuples);
    OCICommit($db_conn);
}
...
function handleSelectRequest() {
    ...
    $query = "SELECT * FROM provides WHERE" ;
...
function handleDeleteRequest() {
    ...
    $query = "DELETE FROM provides WHERE moverID='" . $moverid . "'";
    executePlainSQL($query);
...
if (isset($_POST['select']) || isset($_POST['insert']) || isset($_POST['update'])|| isset($_POST['delete'])) {
    ...
    if(connectToDB()) {
        $result = executePlainSQL("select * from provides");
        printResult($result);
    }
