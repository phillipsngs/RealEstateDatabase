...
function handleUpdateRequest() {
    ...
    //
    if ($nummovers != "") { 
        $query = $query . " nummovers='".$nummovers."'";
    } else if ($trucksize != "") { 
        $query = $query .  " trucksize='".$trucksize."'";
    } 
    ...
    // executePlainSQL("UPDATE pricing SET name='" . $new_name . "' WHERE name='" . $old_name . "'");
...
function handleInsertRequest() {
    ...
    $tuple = array (
        ":bind1" => $_POST['nummovers'],
        ":bind2" => $_POST['trucksize'],
        ":bind3" => $_POST['price'],
    );
    ...
    executeBoundSQL("insert into pricing values (:bind1, :bind2, :bind3)", $alltuples);
    OCICommit($db_conn);
}
...
function handleSelectRequest() {
    ...
    $query = "select price, numMovers, truckSize from pricing WHERE" ;
    ...
    if(isset($_POST['attribute'])){
        ...
        if($selectAttribute == "0") {
            ...
            $query = $query . " trucksize ";
        } else if ($selectAttribute == "2") {
            $query = $query . " nummovers ";
        }
    }
...
if (isset($_POST['select']) || isset($_POST['insert']) || isset($_POST['update'])|| isset($_POST['delete'])) {
    ...
    if(connectToDB()) {
        $result = executePlainSQL("select price, numMovers, truckSize from pricing");
        printResult($result);
    }
